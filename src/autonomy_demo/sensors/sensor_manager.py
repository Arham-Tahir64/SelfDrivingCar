...
class SensorManager:
    ...
    def capture(self, tick_id: int, sim_time_s: float) -> SensorFrameBundle:
        ...
        return SensorFrameBundle(
            ...
            front_camera=CameraFrame("front_camera", image, sim_time_s),
            rear_camera=CameraFrame("rear_camera", image, sim_time_s),
            left_camera=CameraFrame("left_camera", image, sim_time_s),
            right_camera=CameraFrame("right_camera", image, sim_time_s),
            lidar=LidarFrame(points_xyz=lidar_points, timestamp_s=sim_time_s),
            radar=RadarFrame(detections=radar, timestamp_s=sim_time_s),
            gnss=GnssReading(
            ...
            semantic_camera=CameraFrame("semantic_camera", image, sim_time_s),
